...
6. Include an evidence-backed `score` (0–100, at most two decimal places) and nonempty `score_reasons` (each with `dimension`, `reason`, and verified `source_url`). Use the frozen profile and its policy; this is source-level fit inference, not a customer grade or deal probability. Explain unknowns conservatively and never assign a default score or inflate one to reach a research threshold. MCP accepts `name` and maps it to `company_name`, generating stable source/context IDs from the source URL and website host. Direct HTTP runners must supply the canonical fields in the API contract.
7. Submit source records in batches of at most 20 with the current `agent_id` and lease token. This task-scoped endpoint is the only allowed write path. Use the claim response's `attempt_count` in a stable request key such as `job-{job_id}-attempt-{attempt_count}-batch-{n}`. Retry the exact same payload with the same key after a lost response; a later reclaim has a new attempt number and must not reuse a prior attempt's key.
8. Treat Ark's returned `customer_ids` as the only customer identities. A domain is external identity evidence and a deduplication signal, never the business master key. Ark may create a customer, append the source to an existing customer, or quarantine a conflicting source; preserve its `created_customers`, `appended_sources`, `quarantined_sources`, `customer_ids`, and `research_task_ids` result instead of inventing legacy lead/company/profile IDs. Never create a competing research run for a returned `research_task_id`.
9. Continue until the target count is met or credible sources are exhausted.
10. Mark the job complete only after every accepted batch is acknowledged. On HTTP 422, inspect the returned field paths and validation types; correct the indicated fields instead of changing dates at random. Never submit fabricated test companies to a real task. A changed payload needs a new batch key; reserve retries under the same key for identical payloads. Mark it failed with a registered operational error code while the lease is still valid if browsing or API access prevents useful results.
