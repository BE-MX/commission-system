...
- `backend/alembic/versions/099_di_message_interact.py`: message interaction JSON migration with revision ID `099_di_message_interact`; revision number must be rechecked across all branches immediately before creation.
...
**Files:**
- Create: `backend/alembic/versions/099_di_message_interact.py`
...
cd backend
.\.venv\Scripts\python.exe -m alembic heads
.\.venv\Scripts\python.exe -m alembic upgrade 098_customer_image_portal:099_di_message_interact --sql
.\.venv\Scripts\python.exe -m pytest tests/test_design_image_models.py tests/test_design_image_interactions_api.py -q
```

Expected: one head, exact 098→099 MySQL DDL renders without inspection errors, and all focused tests pass.
...
3. set only the child Alembic process's `COMMISSION_DB_HOST/PORT/USER/PASSWORD/NAME`, upgrade the empty schema to `098_customer_image_portal`, then upgrade exactly to `099_di_message_interact`;
...
```powershell
git add backend/alembic/versions/099_di_message_interact.py backend/app/design_image/models.py backend/app/design_image/schemas.py backend/app/design_image/router.py backend/tests/test_design_image_models.py backend/tests/test_design_image_interactions_api.py scripts/test_di_migration_mysql.ps1 docs/database.md docs/api-reference.md
