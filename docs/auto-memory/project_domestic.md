...
- 128 起订单头的 `normal/special` 字段正式改名为 `order_category`；新 `order_type` 与 `order_channel` 是两个独立的 `sys_dict` 值域，新订单必填。历史行允许两列为 `NULL` 并统一展示“未填写”，不推断、不回填，也不保留旧 `order_type=normal/special` 接口兼容。
...
- `domestic_attribute_cutover` 默认只做 JSON 预检，显式 `--apply` 才替换受管标准字典与标准工艺映射；必须与迁移 128 和新版应用部署处于同一停写维护窗口。命令保留 `_special` 数据且不改历史产品、订单、明细、属性或路线快照。
- 历史订单是否清空是后续独立破坏性决策；迁移 128 与属性 cutover 都不得夹带清库，执行清空前必须另行取得明确授权。
