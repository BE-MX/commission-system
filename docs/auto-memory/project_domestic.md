...
- 129 起订单头的 `normal/special` 字段正式改名为 `order_category`；新 `order_type` 与 `order_channel` 是两个独立的 `sys_dict` 值域，新订单必填。每次编辑都按请求值/现存值合成最终二元组并重新校验两项非空、启用；历史两列都为 `NULL` 时必须一次补齐，不能只改备注或只补一维。历史行读取仍统一展示“未填写”，不推断、不回填，也不保留旧 `order_type=normal/special` 接口兼容。
- 头套和发片使用分离字典：头套有工艺、发长、可选网帽颜色、尺码、发型系列，只有 15厘米头套显示并保存发量；发片把工艺/尺寸合并存到 `craft`，仅再保存发长。现有自由文本和参考图发型要求继续保留。
- 普货只接受启用标准值，并要求大小写精确，标准 `SS` 不接受 `ss`；订单类型和渠道同样精确匹配。特单输入若仅大小写不同，先统一为启用标准字典中的 canonical `code`，否则统一为已有启用 `_special` 项的 canonical `code`，再进入工艺映射和产品沉淀；不存在时才在订单/草稿同一事务创建 `_special` 项。唯一键竞争的启用胜方也按其 canonical `code` 复用，停用胜方明确拒绝。MySQL 对字典、工艺映射和产品键查询使用二进制精确条件，并在取行后再次核对原始字符串；下游大小写碰撞不得回收错误行。特单自定义工艺仅在映射缺失时继承产品类型默认路线；已有映射（含人工配置的非默认路线）直接沿用，绝不因默认路线状态或唯一键竞争而覆盖。SQLite 下单/追加的读后写事务以 `BEGIN IMMEDIATE` 在读取前串行化写者，MySQL 锁与竞争处理不变。
- 主站客户远程搜索以单调递增请求编号防乱序，只有最新请求可更新候选列表和 loading；小程序订单列表/详情、速查、客户进度和内贸报工确认弹层都显示订单类别、类型、渠道，特单徽标只看原始 `order_category`。
- `domestic_attribute_cutover` 默认只做 JSON 预检；真实执行必须同时传 `--apply --confirm-writes-stopped DOMESTIC_WRITES_STOPPED`，且 token 不能代替真实停写和在途事务排空。命令替换受管标准字典与标准工艺映射，保留 `_special` 数据且不改历史产品、订单、明细、属性或路线快照；必须与迁移 129 和新版应用部署处于同一停写维护窗口。
