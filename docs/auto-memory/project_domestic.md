...
- 普货只接受启用标准值。所有内贸字典 `code` 必须大小写精确匹配，MySQL 用二进制条件并对结果字符串二次核对；标准 `SS` 不接受 `ss`，特单可把 `ss` 作为自定义值，唯一键竞争也不得误拿仅大小写不同的项。特单仅对当前可见属性允许下拉直接输入，新值在订单/草稿同一事务写入对应 `_special` 字典并只供特单复用；订单失败不残留。特单自定义工艺仅在映射缺失时继承产品类型默认路线；已有映射（含人工配置的非默认路线）直接沿用，绝不因默认路线状态或唯一键竞争而覆盖。SQLite 下单/追加的读后写事务以 `BEGIN IMMEDIATE` 在读取前串行化写者，MySQL 锁与竞争处理不变。
