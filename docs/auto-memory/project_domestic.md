...
- 原价唯一键是 `(product_type, craft, length)`；发片尺寸合并进 craft。缺原价的 SKU 允许沉淀但禁止报价/建单。客户端只提交服务端 `expected_quote`，订单服务在锁内重算；价格变化统一 409，必须人工确认并换 request_id。正式订单冻结原价、优惠价、优惠额、会员、规则及版本，不追随以后充值或改价。
