...
### 2026-09-16 出库单打印与 Word 规则

- 规格中的 B1/B3 独立放大加粗：HTML 16px、Word 12pt，其余规格文字保持原样。
- HTML 打印数据接口与 Word 导出共用后端排序：规格自然升序（B1天才 → B1平行 → B3天才 → B3平行），同规格按尺寸数值升序。
- 尺寸优先取产品 size，缺失时识别产品名称首个斜杠后的尺寸段；未知尺寸在同规格末尾。同规格同尺寸保持原顺序，打印序号重新连续编号。
- 2026-09-18 起，手机网页与小程序扫码、刷新明细也共用出库单排序；同规格同尺寸保持原相对顺序，照片和视频仍按 item_id 关联。


### 出库单负责人中文姓名

打印与 Word 的负责人显示为“英文名（中文姓名）”。将英文名精确匹配方舟用户名或有效 OKKI 账号绑定的外部显示名，忽略英文大小写及首尾空格，合并两类候选后仅在唯一未删除人员且真实姓名含中文时追加。生产 OKKI 显示名通常为中文，不能仅按该字段匹配英文负责人。跨字段匹配不同人员、无对应关系或已有中文时保留原名，不改变原始出库数据或权限归属。


### 验货单数据范围（2026-09-16）

业务员按出库单客户的 OKKI 归属查看验货单，检验提交人不是业务归属。列表在 SQL 分页和计数前过滤；详情、打印所用详情接口、撤回及照片/视频读取使用相同规则。独立数据权限 `shipping_inspection:inspection_read_all` 可查看全部验货单，原 `shipping_inspection:read_all` 仍仅控制出库单。两项数据权限均需配合模块读/写/管理入口权限；撤回另需写/管理。超级管理员可看全部。新增权限启动登记、人工授予，未直接变更生产角色；权限调整后刷新登录令牌。无有效 OKKI 绑定拒绝本人范围访问，业务归属查询失败不放宽权限。
