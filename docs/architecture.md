...
2026-09-06 本地审查保留模块化单体与现有领域 service 边界，重点修复中央请求状态、路线引用/查询、连接器鉴权与读取超时。主站 API client 统一登记在 `clients.js`；PM 保留独立身份体系。WhatsApp Connector 缺密钥必须启动失败，默认只监听本机。详情与页面覆盖、实际验证范围见上述审查报告；这些本地改动尚未部署，不能据此改写下节生产拓扑。
