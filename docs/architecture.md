...
> 2026-09-26 Agent 拓扑更新：OpenClaw、钉钉群监听、客户/库存/物流/社媒 MCP、中继、OKKI 与 Shopify 配套同步统一运行在北京 `154.8.205.162`，公网入口为 `leshine.cloud`。新加坡只转发旧 Agent URL，实例与业务调度均已退役；办公室/方舟后端归属不变。后续 Agent 服务同样部署北京，详见 [部署约定](../deploy/agent-cloud-migration.md)。

## 云存储改造目标（2026-09-19，本地实现，未启用生产）

公网文件经北京应用接入LighthouseCOS；办公室手机检验保持局域网直达办公室，原件落盘并fsync后，在同一业务事务登记持久同步队列，由实例自己的worker上传并回读SHA256验证。数据库仅保存引用、权限和同步状态。公网中转并不改变仍由办公室负责的业务控制API；各域Nginx切换需逐项验收。

读取采用有容量上限的本机缓存，文件响应持有跨进程租约，防止下载期间被淘汰。应用鉴权后才读取私有对象；短时签名只用于允许的下载流程。LighthouseCOS现场CORS/OPTIONS受限，当前实施不依赖浏览器直传或cosfs挂载。

色块工作台的R2接口通过服务端适配器访问同机FastAPI存储网关，稳定逻辑键由SQL别名映射到COS不可变对象。R2仅继续承载未完成分片及重试回执；完成操作在SQL内幂等，避免旧上传重试覆盖新版文件。已有D1业务引用不变。

生产状态、迁移进度和阻塞只维护在 `docs/handoff.md`；本文描述目标结构，不表示已部署。
...
莱莎方舟平台是企业内部综合中台，采用 FastAPI + Vue 3 的模块化单体。业务覆盖提成/薪资、发票/回款、外贸与内贸生产履约、备货、售后、物流、设计/素材/发色、生图/AI 对话、展会/名片、客户经营、知识库、培训、报表与集成运维；PM 协作站、客户门户和小程序具有各自的入口与鉴权边界。模块、API 与页面的当前源码覆盖清单见 [2026-09-06 平台审查](requirements/2026-09-06-platform-audit.md)，不再用历史固定模块数代表现状。

> 「内贸」在本仓库有两个不相干的含义，别混：**内贸品牌**指展会试戴用的「莱莎健康假发」品牌线；**内贸订单管理**（`app/domestic/`）指国内订单的下单与按数量拆批报工。

2026-09-06本地审查保留模块化单体与现有领域service边界，重点修复中央请求状态、路线引用/查询、连接器鉴权与读取超时。主站API client统一登记在`clients.js`；PM保留独立身份体系。WhatsApp Connector缺密钥必须启动失败，默认只监听本机。当次验证范围见上述历史审查报告；后续发布状态以[交接文档](handoff.md)及对应发布证据为准，不从历史“本地完成”推断当前线上状态。
...
| 定时任务 | APScheduler | 开关与时区由 Settings 控制，任务由 registry 注册；实际任务与单活实例以运行与自动化中心和部署登记为准 |
...
新增领域沿用同一注册入口：`app/receipt/`负责方舟回款/凭证与小满发送状态，`app/announcement/`负责公告/周报及知识库关联；接口和表结构分别维护在[API参考](api-reference.md)和[数据库参考](database.md)，此处不重复清单。
...
`https://lan.leshine.cloud` 解析到 192.168.100.3（2026-09-20 已同步公共 DNS、办公室网关 DNS、Caddy 监听与防火墙），由独立 NSSM ArkOfficeHttps/Caddy 提供443并代理现有8001。仅放行192.168.100.0/23，未新增公网端口映射；原业务服务不变。证书到期2026-12-15，手动DNS-01续期，尚无自动续期。配置及恢复约束见 deploy/README.md 的办公室内网HTTPS专项入口。证书健康和匿名鉴权已验证，物理手机扫码未验收；办公室尚未部署新station接口，HTTPS启用不代表新出库检验页已上线。
