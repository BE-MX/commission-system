...
| 翻译被限流 | `quota_minute` / `quota_daily` | 按 30 请求/分钟/设备与 200,000 输入字符/北京时间日解释；不做临时改库解锁。 |
| 翻译不可用 | `preset_disabled` / `provider_failed` | 先检查 AI Preset 是否启用，再检查 Provider 健康；`provider_failed` 可重试。 |
| 版本阻断 | `version_too_low` | 让用户从 Ark 管理页下载最新 ZIP；禁止用旧包替代。 |
