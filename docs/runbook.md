...
可选模型使用独立 Preset，禁止覆盖默认 Preset 的 model：GPT Image 2.5 为 `design_image_generation_GPT_image_25 / gpt-image-2.5-sunburst`（preset 名大小写敏感，model ID 取自 openlux lab 精确 ID），与 Grok 一样绑定 openlux `https://api.openlux.ai/v1`；Grok Image 2 为 `design_image_generation_grok_image_2 / grok-imagine-image-2.0`，绑定 openlux `https://api.openlux.ai/v1`；Nano Banana Pro 为 `design_image_generation_nano_banana_pro / gemini-3-pro-image`，Nano Banana 2 为 `design_image_generation_nano_banana_2 / gemini-3.1-flash-image`，仍绑定 TeamRouter。Grok 的旧 `grok-image-2` 占位 ID 已删除，不保留别名。
