...
用户授权合并推送本轮返回主页、钉钉PDF、验货查询和发票备注映射。基于远端main 16fbb879，在codex/shipping-notice集成689a5265；冲突仅为文档新增章节，全部保留。候选67项相关测试通过、增量约定检查无违规；前端与此前已构建验收的e38a4f52完全一致。验证后在主目录合并并推送main；不部署，不改生产数据。测试截图、PDF样张和浏览器脚本收尾保存到主目录 `.deploy_state/shipping-improvements-20260917/`。
