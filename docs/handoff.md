...
- **数据库迁移数**：076（Alembic head `076_pm_hub`，2026-07-18；073/074 发票配件、075 培训速递、076 PM 站）
- **用户数**：~30 人（莱莎员工）
- **日活**：~20 人
...
- **总计 532 tests（2026-07-13 全绿）→ 753 tests（2026-07-18 全绿，培训速递/PM 站/发票配件合入后；PM display_name 断言已随 seed 改名修复为从 MEMBERS_SEED 派生）**
...
5. **展会夏季衣橱 + 反转镜头待验证合并**（2026-07-18）：夏季着装提示词子句（换装/场景路径统一夏装、禁品牌 logo）+ kiosk 拍照页前/后置切换，在分支 `claude/expo-summer-capture`（已推送未合并），展会前实测通过后合入 main
