...
亮哥已授权合并并推送 main，包含 `d27d0c98` 内贸列表密度优化及 `9259091c` 逐件码日期放大。从 `b61e1a4f` 快进集成，核验业务代码与已验证版本一致后推送。证据归档至主目录 `tmp/domestic-density-date-delivery/evidence/`，远端核验后清理本任务 worktree 和已合并本地分支。本轮不含应用部署。
