...
亮哥已授权合并推送。本次集成最新 main `ac7d35eb`，只处理交接文档新增记录冲突并保留双方内容，业务修复与已验证版本一致；合并后资格队列/客户工作流/公海研究专项 96 passed。约定检查被 AssetTagEditor.vue 既有 small 按钮和 AssetLibrary.vue 行数债务基线失配阻挡，干净 main 同样复现，本次未改资产前端。此次不部署应用，也不重复执行 9 月 9 日的任务激活。
