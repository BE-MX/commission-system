### 展会生图提示词配置与版本（2026-09-07，本地待交付）

- 工作分支 `codex/expo-prompt-config`，worktree `D:/MyProgram/commission-system-codex-expo-prompt-config`。已实现管理页、动态版本选择、原子快照、版本修订冲突保护与管理员历史快照查看。
- 139 迁移新增版本表及结果快照。真实/柔光/美颜完整迁移，78 个历史生成组合文本哈希一致。生产尚未迁移或发布；线上仍为此前回滚版本。
- 验证：最终直接运行全部 test_expo_*.py，425 项通过（含版本专项 97 项）；前端动态版本、kiosk 隔离与导航回归 30 项通过。前端生产构建和增量约定检查通过。管理页浏览器验证创建、未保存预览、保存、生效列表及 390px 布局，X/Escape/遮罩取消放弃保留草稿，保存中阻止关闭。独立审查发现的关闭保护与历史快照入口已处理。
- 发布前必须停止接收新生成并排空旧线程，由指定部署入口统一执行 schema 139，再同步前后端并刷新设备。只有 SQLite 迁移与业务实测，MySQL 锁并发未实测。需要用户明确授权后才合并、推送或生产发布。
