...
- 工作树 `D:/MyProgram/commission-system-codex-presale-settlement`，分支 `codex/presale-settlement`。未提交/推送/合并/部署，未操作共享数据库。预售迁移顺延为166（父164），避开主目录未提交跟单员迁移165；后者集成时需重接已发布迁移链。
