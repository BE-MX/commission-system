## 2026-09-11 WhatsApp v1.6.0 自动接管（本地交付）

用户授权当前聊天主动开启后自动生成并发送。话术旁新增开关，沿用后台生成预设，模型选择回复/等待/交人工，最多3段短消息。当前前台一对一聊天生效；人工输入、切聊天/后台、发送不确定等停止；浏览器同站点单实例锁。独立审查发现的采集自失效、其他设备抢先回复、发送不确定被新消息覆盖、待开启竞态和最新边界检查均补回归。后端101通过/1跳过，扩展250单测，23条不同合成浏览器路径覆盖；真实WhatsApp和真实模型未测。必须配套更新后端auto_reply_enabled契约，无迁移。未合并推送部署。详见 [使用和边界](requirements/2026-09-11-whatsapp-auto-takeover.md)。
