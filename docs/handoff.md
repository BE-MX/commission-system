...
> 🚧 **统一客户经营重构（2026-08-31，待维护窗切换）**：功能分支 `codex/unified-customer-profile` 已完成迁移 126 及客户主档、身份解析、事实证据、档案版本、Agent 上下文、公海背调、搜索任务、客户池、机会台、经营雷达、受控提案、MCP 只读工具和五个前端入口。方舟是唯一真相源；公司名可空且不作身份键；外部来源经 Agent 事实化后进入方舟，消费 Agent 只读方舟。旧 `ark_sales_companies/contacts/research_*`、旧公海任务和旧客户画像运行时已退役。迁移冻结 39 张表、778 个字段，并在隔离 MySQL 8.4.11 严格模式实建核验：表/字段空备注均为 0；过程中修复了 `rank` 保留字 CHECK 和 MySQL 8.4 反射归一化问题。已使用主工作树受控 `.env` 对腾讯云共享库完成停写前只读 preflight：当前版本为 `125_invoice_integration`，22 张待处理旧表库存哈希为 `74fa675c283fb105c6b113c502495b2b0c5c23605b377e2e00631c2c4fb65df7`，证据位于本 worktree 已忽略的 `backend/tmp/customer-domain-cutover/preflight-20260831T134024.json`。生产仍未切换：北京云 `ark-backend`、新加坡 `okki-sync`/OpenClaw 和办公室 `192.168.101.193:8001` 均在运行；当前机器能访问办公室业务端口，但 22/5985/445 管理端口不可达。应用与迁移当前还共用全局高权限 `root@%`，且 MySQL `partial_revokes=0`，不满足“迁移 principal 与全部 writer principal 分离并撤销写权限”的硬门禁。还缺 RDS 可恢复快照 ID、权威抑制源、办公室现场停服/部署入口、独立迁移与应用账号、停止 writer/撤权证明及五分钟批准标记；这些材料齐备前禁止清库或应用 126。代码完成不等于生产完成。

> 2026-08-31 后续复核：腾讯云主账号 Chrome 登录态已经恢复，可进入 TDSQL-C 集群列表；目标连接域名携带集群标识 `cynosdbmysql-grp-4lsjlqyi`，地域为北京。腾讯云复杂控制台在自动读取和地域切换时连续超时，尚未创建手动快照，也没有取得备份任务 ID。数据库 `information_schema.PROCESSLIST` 同时仍显示办公室公网 IP `123.235.152.38` 对 `commission_db` 保持 9 个 `root` 连接，最长 Sleep 仅 5 秒，证明办公室 writer 尚未停写。不得把“已登录腾讯云”误当成“已备份/已停写”。
