### 展会合成购买意向与适度美颜（2026-09-07）

任务分支 `codex/expo-purchase-prompt`：亮哥确认真实、柔光、美颜三个版本都允许适度美颜，并授权合并至 `main`、推送 GitHub `origin/main`。已在三条合成路径加入基于本人长相、增强购买意向的共同目标；真实/柔光改为轻修，美颜改为适度精修，保留本人辨识度、年龄特征、面部比例与发丝细节；同步版本说明与模块文档。

验证：后端 `pytest tests/test_expo_color_scene.py -q` 97 项通过（SQLite 隔离 fixture / 纯提示词组装，无生产写入）；前端版本接线 7 项通过，`npm run build` 通过（既有混合导入与大包警告）；`check_conventions.py` 无新增违规，`git diff --check` 通过。`git_sweep.py --no-fetch` 已完成，仅报告本地快照中的现有未提交/未推送事项，未修改其他分支。交付范围为代码合并与推送，未执行生产部署或收费生图验证，线上提示词尚未改变。
