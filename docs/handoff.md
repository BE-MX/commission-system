...
- **数据库迁移数**：048（Alembic，最新 `048_expo_hair_colors`）
- **用户数**：~30 人（莱莎员工）
- **日活**：~20 人
...
20. ✅ **展会 AI 试戴**（2026-07-03，内贸品牌「莱莎健康假发」，8 月展会用）：
    ...
    - 2026-07-07 全链路实测迭代：**图像模型已接入**（ELBNT `/v1/images/edits`，`expo_wig_composite` 启用）；发色库独立表（048，色板图+描述，三图合成）；匹配屏单选发型+可选生成场景（原景/居家/办公/聚会）+ AI 面容解读展示；魔法镜框动效 + 黑金 LOGO + 新广告语；结果页二维码卡片化+手动返回（不自动清场）；稳定性四件套——性别过滤全灭兜底、卡死看门狗（pending>180s/generating>420s 自愈）、AI 非法 JSON 纠错重试、参考图送模型前统一压缩（16MB→155KB）；生图超时下限 300s
    - **待完成**：单场景合成在上游拥堵时段仍可能超 300s 被 ELBNT 网关 5xx（2026-07-07 晚多次实测），需持续观察或议价换 Provider；发型库实拍图与 6 个月对比素材待市场部提供；kiosk 品牌绿换肤待做
