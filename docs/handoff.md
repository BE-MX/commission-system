...
23. ✅ **内贸订单管理**（2026-07-27 合入 main 并已上生产；需求稿 docs/requirements/2026-07-27-domestic-orders.md）：
    ...
    - 手机/PDA 浏览器备用版：主站全屏路由 `/domestic/reporting`，复用相同登录和 `/api/mini/domestic/*` 后端；支持 HTTPS 后置摄像头、扫描头键盘模拟与手输，数量拆批、逐件自动报 1 件、今日记录/撤销、参考图和弱网幂等恢复；构建额外生成 Android 6 / Chrome 49 兼容包，摄像头不支持时自动降级但不阻断扫描头
