...
> 🚧 **内贸条件工序（2026-08-31，代码就绪≠生产已切换）**：分支 `codex/domestic-conditional-routing` 已实现内贸 `required / decision / optional` 三类规则、按具体单件的分流与自动/人工跳过审计、全后序撤销保护，以及 Web/小程序/PDA 共用的 outcomes 契约；迁移文件为 `127_domestic_route_rules`。当前只表示代码和自动化契约就绪：**真实 MySQL 尚未执行 127，生产工艺映射/存量产品/在制明细均未切换，受控 cutover 脚本也未在真实库 apply**。上线先在 schema 维护窗口停旧写实例、升级并核对唯一 Alembic head，只启动新版本。应用验证后另开业务切换维护窗口：停止全部内贸写实例/后台写任务并等待在途事务排空（只读可保留），此后才 dry-run、人工复核 token 与 reported-item reconciliation，并带 `--confirm-writes-stopped DOMESTIC_WRITES_STOPPED` 单事务 apply；成功核对后再恢复写流量。行锁不能替代此停写门禁。真实 MySQL 两连接下的 InnoDB 等待/锁序并发仍是发布前门禁；SQLite 只能证明状态语义。全仓 `knowledge` 相关既有失败若仍复现必须单独留证，不能为让本功能变绿而改基线或掩盖。
