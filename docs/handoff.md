## 2026-09-11 WhatsApp v1.6.7 语言中文标记（本地修改）
...
分支codex/whatsapp-testing-label，基点23e4c994。自动接管入口改为“自动接管（测试中）”，同步初始渲染、停止后恢复文案及既有浏览器用例。开启后仍显示“停止接管”。仅扩展文案变更，无后端变更；构建283单测及窄屏/开关2条浏览器路径通过。约定检查仍为素材库两项既有问题；未合并推送。
