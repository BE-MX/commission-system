...
> 🚧 **内贸条件工序（2026-09-01，schema 已上生产，业务映射待切换）**：`main` 已实现内贸 `required / decision / optional` 三类规则、按具体单件的分流与自动/人工跳过审计、全后序撤销保护，以及 Web/小程序/PDA 共用的 outcomes 契约；真实 MySQL 已执行并验收 `127_domestic_route_rules`。最终业务口径是不迁移存量订单：`cap`（头套）的全部工艺映射和已有产品直接切到“头套网帽（递针）”，`piece`（发片）直接切到“发片网底（递针）”；两条路线使用完全相同的三个 decision 和一个 optional 条件契约，已有订单明细和进度保留路线快照。`domestic_route_cutover.py` 默认只读检查两条路线、启用工艺字典、工序、条件规则和在职人员覆盖；apply 自动补齐缺失工艺映射后更新映射与产品，不删除或重建订单。**生产规则配置和 apply 尚未执行**；执行前仍须停止全部内贸写实例、后台写任务和路线/工艺配置写入并排空在途事务，成功核对后再恢复写流量。真实 MySQL 双连接的锁与整笔回滚仍需在生产切换前验收。
