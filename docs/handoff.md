### 登录页背景与动效优化（2026-09-06，本地验收完成，待合入 main）

工作目录 `commission-system-codex-login-background`，分支 `codex/login-background-polish`。第三轮按当前导航和已实现模块重写文案：定位“AI 驱动的企业协同平台”，总结“贯通业务 · 沉淀知识 · 智能协同”，功能概括为“客户经营 · 产销履约 · 业绩核算 · 创意设计 · 知识洞察”；登录卡欢迎语同步。完成黑金背景收敛、静态渐变登录卡和 280ms 入场；按亮哥第二轮反馈把地图从下方移到页面中部，标题叠在地图上，加入左向渐变粒子尾迹与轻微航行浮动，补齐原先中部留白。尾迹使用 38 个确定性错峰粒子与五条静态流线；减少动态模式保留流线并关闭持续运动。修正本页密码按钮白底、选项间距，使用原生勾选框并补输入/显隐无障碍标签。认证接口、登录后跳转与生产环境未改。

第四轮按亮哥反馈采用 Natural Earth 实际海岸线（本地 public domain 资源）替换手绘大陆多边形，增加淡填色、经纬线与点阵；青岛核心放大至半径 4.5px，增加半径 34px 静态光晕、轻微呼吸与两层扩散环，减少动态/窄屏保留静态光晕。地图大陆判定移出逐帧循环，使用双 canvas 分离静态地图与航线光点；修正轨迹每帧改变起点、按帧累加导致速度随刷新率变化的问题。支持即时减少动态、窄屏静态、后台暂停、容器缩放与卸载清理。颜色归入 `--login-*`，只减少这两个组件已清理的裸颜色债务基线。

验证：地图行为回归 6 项（含青岛扩散环同心、随时间扩大及减少动态保留静态节点）与备案回归 3 项通过；浏览器检查 320×568、390×844、844×390、1024×768、1280×720、1920×1080，无横向溢出、备案与卡片不重叠，横屏短屏可滚动。空提交反馈、密码显隐、勾选交互通过。前端构建通过（既有大包/混合导入警告）。全局约定检查仍被未修改的 `DomesticOrders.vue` 行数债务基线失配阻断，需在该模块任务处理；不扩大本次基线。未启动后端，真实账号登录未验；预览初始化的 refresh 请求因本地后端不可用返回 500。登录页变更已完成本地验收；亮哥已授权合并并推送 origin/main，生产部署未在本次授权范围内。
