...
回滚前只读核实：北京 `leshine.cloud` 后端运行提交 `639f3f78`，服务 active，已包含本次新提示词。本轮包含代码回滚、合并推送和通过统一入口回滚北京线上版本；不修改数据库结构、上传照片或历史生成结果。回滚代码与 `a28bbf4e` 对应三个代码/测试文件完全一致，92 项后端测试、7 项前端接线测试、约定检查通过。线上发布结果待收尾记录。
