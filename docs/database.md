...
- **订单发票（10 张业务表，另有 3 张 108 迁移备份表；044 迁移 + 049 扩展 + 107 代创建授权 + 108 北京时间 + 119 截图来源 + 154 出库任务队列）**：
  ...
  - `ark_okki_outbound_tasks`（154）— OKKI 销售出库单自动生成任务队列（invoice_id FK CASCADE, order_id UNIQUE=OKKI 订单 ID, status pending/running/done/failed/skipped/uncertain, reason, attempts, last_error, processed_at）。发票首推小满成功即入队，singapore okki-sync 轮询器消费执行受管 okki_outbound_creator.mjs；含未建品非标合并行或实时核查已有关联出库单的任务落 skipped；提交结果不确定落 uncertain，禁止自动重发；`okki_outbound_reconcile` 对账 job 只补窗口内首推（按首张 create 成功日志判定）缺行的订单；总开关 `OKKI_OUTBOUND_AUTO_ENABLED`
