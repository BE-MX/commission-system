...
- **订单发票（8 张表，044 迁移 + 049 扩展）**：
  ...
  - `ark_invoice_items` — 发票明细（invoice_id FK CASCADE, `product_kind` hair/accessory（073）, item_type stock/custom, product_id 可空, sku_id, custom_product_id, product_name/display, net_weight_grams/curl/length 对配件可空, model 可空, color, quantity, standard_price+customer_price 双价快照, price_per_piece, discount_amount 行级折扣负数/0——070 迁移, total_price=单价×数量+折扣, price_source customer_rule/manual/missing_std, xiaoman_unique_id）。hair 保持原规格校验；accessory 仅使用 Name/Model/Color 与真实 product_id+sku_id。
