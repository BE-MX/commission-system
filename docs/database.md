...
- **订单发票（10 张业务表，另有 3 张 108 迁移备份表；044 迁移 + 049 扩展 + 107 代创建授权 + 108 北京时间 + 119 截图来源 + 154 出库任务队列）**：
  ...
  - `ark_okki_outbound_tasks`（154）— OKKI 销售出库单自动生成任务队列（invoice_id FK CASCADE, order_id UNIQUE=OKKI 订单 ID, status pending/running/done/failed/skipped/uncertain/waiting_stock, reason, attempts, last_error, processed_at）。发票首推小满成功即入队，singapore okki-sync 轮询器消费执行受管 okki_outbound_creator.mjs；含未建品非标合并行或实时核查已有关联出库单的任务落 skipped；明确库存不足落 waiting_stock，每15分钟按目标仓库/SKU复查，不受普通失败次数限制；提交结果不确定落 uncertain，禁止自动重发；`okki_outbound_reconcile` 对账 job 只补窗口内首推（按首张 create 成功日志判定）缺行的订单；总开关 `OKKI_OUTBOUND_AUTO_ENABLED`
...
迁移 `160_domestic_price_review` 新增 `ark_domestic_order_items.default_discount_price DECIMAL(14,2) NULL`，保存系统默认商品优惠价，供审核比较，独立于手工成交价且不含手工费。新报价写入；历史 NULL 使用已保存系统价或按原价、会员及属性快照重建人工价的默认对照。新增 `sys_dict.domestic_order_type` 的 `sample=样单`，保留已有同编码配置。迁移不更改历史订单状态、金额或客户等级，生产由统一部署入口执行。
