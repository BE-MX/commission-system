...
- 特单自定义属性：只有 `order_category=special` 时，当前产品类型和条件下**可见**的属性下拉允许直接输入新值；新值保存到对应 `_special` 字典，只在后续特单中复用，普货下拉不展示且后端拒绝非标准值。自定义项仅在草稿或订单保存事务内创建，订单失败会一并回滚，不提供脱离订单的即时创建端点。自定义工艺首次出现且尚无映射时，自动映射到该产品类型默认路线（头套“头套网帽（递针）”、发片“发片网底（递针）”）；只有此时才校验默认路线，缺失、停用或无工序则拒绝保存。已存在的自定义工艺映射是管理员配置的真相源，后续特单直接沿用，不因默认路线状态而校验或覆盖；唯一键竞争后也读取并保留胜方映射。特单切回普货只清空非标准值，产品类型或发长变化会清空已不适用的条件属性。
