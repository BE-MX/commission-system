...
```powershell
python -m scripts.domestic_attribute_cutover --apply
```

`--apply` 在一个事务内重新校验门禁、完整替换受管标准字典及标准工艺映射、保留 `_special` 字典和非标准特单映射，并在提交前验证计划已收敛。命令不会删除或改写历史产品、订单、订单明细、属性快照和路线快照；失败整笔回滚，重复执行应无新增变化。历史订单数据本次不清理；如后续确需全部清空，必须另行取得明确授权并使用独立的破坏性操作。
