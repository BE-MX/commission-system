...
- 订单数据范围与导出（2026-09-07）：`GET /orders`、`GET /orders/{id}`、`GET /orders/{id}/export` 默认只允许创建人访问；`domestic:read_all` 或 super_admin 可查看全部，但该数据范围码不扩大操作权。草稿提交会把正式订单日期同步为客户最近下单日期。业务 Excel 只显示客户编码（缺失显示“未填写”，不回退名称），显示原价、优惠金额（实际减免额）、优惠后商品单价、手工费和小计；两类订单的数量后均有空白“入库数量”。参考图嵌入对应产品/字段单元格，长文字续到“完整要求”，图片不可用时明确标记；两表均为 A4。生产单保持无金额。
