...
## 内贸订单（`/api/domestic`，081/082/116/127/129/130/131/132 迁移，2026-07-27 至 2026-09-02）
...
- 客户：`GET /customers`（分页 keyword/status）返回派生会员标签、最近充值金额/时间与余额；`POST /customers`、`PUT /customers/{id}`、`DELETE /customers/{id}` 不接受手工会员等级。会员只由**最近一次成功充值金额**决定，与历史充值合计和当前余额无关：`[10000,30000)` 银卡、`[30000,100000)` 黑卡、`>=100000` 至尊，低于 10000 为普通客户。`POST /customers/{id}/recharges` 的 body 为必填 `amount/request_id` 和可选 `remark`；同一客户同一 `request_id` 同内容只重放首次结果，不重复入账，改内容拒绝。`GET /customers/{id}/balance-ledger` 查余额流水；两者需 `domestic:recharge` 或 `domestic:admin`。
