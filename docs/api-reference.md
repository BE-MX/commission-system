...
- `/api/expo` — 展会 AI 假发试戴（`expo/router.py`，需 `expo:read/write/admin`；`GET /share/{code}` 分享落地页无鉴权）
  ...
  - 选项端点：`GET /hair-colors`（发色库列表，`?only_active=0` 管理端取全量；048 起独立表 ark_expo_hair_colors，不再复用 ark_color_palette）、`GET /scenes?mode=scene|tryon`（scene=场景大片五景 / tryon=试戴生成场景：白领高管/老师/老板娘/公务员/医生/居家/聚会 7 景，key/label/tagline；tryon 额外返回 `image` 示意图 URL——探测 uploads/expo/scenes/&lt;key&gt;.* 存在则给 /uploads 路径否则 null，仅供 kiosk 滑动选择器示意、不参与合成；tryon 统一输出 6 寸竖版 1024x1536，尺寸经 `size` 参数下发生图接口。multi 多场景合一已于 2026-07-09 下线）
