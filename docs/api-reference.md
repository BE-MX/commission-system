...
- `/api/expo` — 展会 AI 假发试戴（`expo/router.py`，需 `expo:read/write/admin`；`GET /share/{code}` 分享落地页与 `GET|POST /upload/{token}` 扫码上传页/收图同样无鉴权，令牌即凭证）
  - 生图提示词版本（139）：`GET /prompt-versions/picker`（expo:write，仅启用版本名称/说明/默认标记/修订号）；其余接口均 expo:admin：`GET /prompt-versions`（keyword/page/page_size 分页）、`GET /prompt-versions/editor`（编辑分区、允许占位符、场景目录）、`GET /prompt-versions/{id}`（完整配置）、`POST /prompt-versions`（name/hint/is_active/config）、`PUT /prompt-versions/{id}`（同创建并带 expected_revision，冲突 409）、`POST /prompt-versions/{id}/default`（expected_revision；默认版不可停用）、`POST /prompt-versions/preview`（config/mode/scene_key/wig_id/hair_color_id；只预览文本，无 AI 调用/扣额）。`GET /results/{id}/prompt-snapshot` 仅管理员查看实际提示词；历史无快照返回 data=null。普通会话 results 仅返回 `prompt_version={id,name,revision}`，即使 internal=1 也不含正文。
  - 试戴主流程：`POST /register`（consent 必填；`phone` 服务端归一后校验 11 位——NFKC 折全角、剥非数字与 +86 前缀，落库为纯数字，检索与脱敏都依赖这个口径）→ `PUT /customers/{id}`（kiosk 返回上一步改登记信息，不重复建档，expo:write）→ `POST /sessions`（`?mode=tryon|scene`；照片来源二选一：`photo` multipart 文件（现场拍照）或 `pending_photo` Form 字段（扫码上传的待取文件名），必须恰好提供一个；tryon 异步分析+匹配，scene 直接就绪）→ `GET /sessions/{id}`（轮询统一载荷，`?internal=1` 含内部发况与话术，仅销售面板用；results 每项含 `image_url` 原图 + `display_url` kiosk 压缩展示版（{stem}_disp.jpg 长边1080 q85，合成时同步生成；历史结果无展示版为 null，前端回退原图——2026-07-14 隧道带宽治理）；`ai_issue` 公开 AI 异常态：超时/408/429/502/503/504/传输错误最多自动重试 3 次，重试中为 `state=retrying`，非超时直接报错或重试耗尽为 `state=contact_admin` 并返回门店优先的 `admin_phone`）→ `POST /sessions/{id}/generate`（tryon：`wig_ids` 单选发型 + 可选 `hair_color_id` 发色 + 可选 `scene_key` 生成场景；scene：`scene_keys` 场景列表；两模式共用 `prompt_version_id` 数据库版本 ID；不传使用数据库当前默认版本。提交前读取最新配置并保存最终提示词快照，配置无效/停用不扣额度；旧 `prompt_variant` 入参已移除（139））→ `POST /sessions/{id}/contact-admin`（仅 `ai_issue.state=contact_admin` 且调用账号与会话属于同一有效门店时可调用；数据库原子幂等通知所有已启用且具 `expo:admin`/`super_admin` 角色权限并已绑定钉钉 ID 的管理员，文案带客户称呼与人脸识别/效果图合成环节）→ `POST /results/{id}/reaction` → `POST /customers/{id}/feedback`
...
## 内贸订单（`/api/domestic`，081/082/116/127/129/130/131/132/133 迁移，2026-07-27 至 2026-09-02）
