...
| POST | `/logo` | multipart `file`；沿用共享图片验证与正规化（真实 MIME、尺寸/像素、最大 20 MiB），保存新的 `customer-logo` 资产并原子切换 current pointer。旧 LOGO 保留供历史任务读取。 |
...
主要错误：邀请不可用统一 401、图片校验 400、资源不存在或越权 404、LOGO 超过 20 MiB 时 413、LOGO 写入过频 429、图片存储不可用 503。公开 API 永不按内部 owner/业务员 scope 判断；其唯一数据边界是当前 active invitation。
