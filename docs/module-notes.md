...
**购买意向与适度美颜（2026-09-07，当前代码口径）**

- 亮哥明确要求「基于用户长相，生成能激发很高购买意向的效果图，既像本人，又有适度美颜」，并确认所有版本都允许适度美颜。
- `_PURCHASE_INTENT_CLAUSE` 覆盖换发原景、换发场景置换和佩戴实拍场景大片，目标是让顾客认出「这是戴上这款假发后更好看的自己」。本人辨识度优先，保留年龄特征、肤色与识别标记，不能为了效果改掉假发款式、密度、纹理或指定发色。
- 真实/柔光从「禁止修皮肤」改为自然轻修：均匀暂时性肤色不匀、减轻临时瑕疵、轻柔淡化细纹，保留毛孔和皮肤质感；柔光另外保留柔和场景光。美颜版适度精修，淡化而不抹除纹路。三版都在修肤之后复锁五官比例，并保护发丝清晰度。
- 前端说明同步为「自然轻修 · 保留本人」「柔光轻修 · 保留质感」「适度精修 · 自然好气色」。版本值和默认真实版不变。以下 08-01 的禁修规则是历史记录，已被本节取代；08-29 的面部几何与有限用光约束继续保留。
- 提示词组装测试可验证覆盖范围与指令一致性；实际美颜程度、本人相似度和购买意向仍需上线后的真实效果评估。发布状态见 `docs/handoff.md`。

**面部神采子句 + 提示词版本开关（2026-08-01，历史规则）**
