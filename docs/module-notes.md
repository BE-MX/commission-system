...
- 客户切换时，前端不复制调价规则；它按 product_id+sku_id 调后端配件价格列表重新解析，丢弃过期客户响应。发票选品和重解析固定传 `active_only=true`；失效 SKU 不进入新增候选，当前失效行引导用户到「价格与产品配置 → 标准价格表」重新配置。价格配置页保持默认 `active_only=false`，继续显示历史配置。
