...
- **版本开关走 AI 预设的 `parameters`，不新建表**：在系统管理 → AI 管理 → `expo_wig_composite` 预设的「调用参数覆盖」里写 `{"face_vitality": "v1"}` 即启用，改成 `off` 或删掉即关闭，下一次生成生效，**不重启不部署**。可选值见 `ai_pipeline._FACE_VITALITY_VARIANTS`。
  - **版本写在代码里、后台只选用哪版**：提示词是这个模块最容易改坏又最难发现改坏的东西，留在代码里才有 review、有测试、能 git blame。后台不提供自由改字。
  - **默认 `off`**，即与 2026-08-01 之前行为完全一致；未知值（打错字）**回落 off 并打日志**，绝不抛异常——展位现场生不出图的代价远大于少一段子句。
  - 该键**不会被发给上游**：`image_service._image_params` 按 `IMAGE_PARAMETER_KEYS` 白名单过滤，与既有的 `api_style` 同一机制；已有测试同时钉住两份白名单，防哪天白名单放宽后静默泄漏。
  - **`expo_wig_composite` 预设不由 `seed_ai.py` 自动创建**（是手工在后台建的），所以这个键也没法自动补进去——新环境要用这个开关，得手动在预设参数里加。
