...
- 客户切换时，前端不复制调价规则；它按 product_id+sku_id 调后端配件价格列表重新解析，丢弃过期客户响应。失效 SKU 要引导用户到「价格与产品配置 → 标准价格表」重新配置。
- 金额口径：头发金额/头发折扣/配件金额/配件折扣为四个只读分组汇总；`product_amount = Σ所有明细净额`；`total_amount = product_amount + Packaging + Shipping Fee + Handling Fee`。行折扣已进 total_price，不再重复扣减；兼容字段 `internal_discount` 仅保存头发折扣。
- OKKI 推单时配件按真实 product_id+sku_id 逐 SKU 逐行推送，`cost_amount` 为含配件行折扣的净额，不合并到通用产品，也不把配件折扣再写入 cost_list。
- Excel/HTML/PDF 导出将头发与配件分成两个明细区，汇总顺序与录入页一致。导出 PDF 启动前需通过字体预检，当前使用项目既有中文字体回退链，禁止静默丢失中文字形。
