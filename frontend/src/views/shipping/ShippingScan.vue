...
const { uploadItemId, uploadError, pendingCompression, retryCompression } = station
...
const uploadFeedbackFor = id => uploadItemId.value === id ? {
  ...
  retryLabel: !busy.value && !invalid.value && !submitted.value && !pendingSubmit.value
    ? pendingCompression.value ? '重试压缩并上传' : pendingUpload.value ? '重试上传' : '' : '',
} : null
