<template>
  <section v-if="['stock', 'presale'].includes(form.order_type)" class="head-section receipt-section">
    <div class="col-title">{{ form.order_type === 'presale' ? '首笔定金' : '本次回款' }} <el-tag effect="plain" type="warning">同步前必填</el-tag></div>
...
watch(() => props.form.receipt_draft, value => {
  if (!value) props.form.receipt_draft = { amount: props.form.order_type === 'presale' ? null : props.total > 0 ? props.total : null,
