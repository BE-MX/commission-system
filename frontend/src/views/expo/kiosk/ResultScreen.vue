  ...
  <div class="result">
    ...
    <div v-if="!current" class="waiting">
      ...
      <div class="wait-title">
        ...
        <!-- 时长按 2026-07-31 晚切 TeamRouter 后实测改口径：单张 21~50s（12 次采样，中位 27s）。
             云雾时代是 165~180s，故本行当天改过两次——**承诺时长必须跟着实测走**，
             换生图 provider/模型时务必连同这行一起复核，别让它变成又一个空承诺。
             写「1 分钟内」而非「约 30 秒」：展位并发时上游会排队，留出余量不打脸 -->
        <small>预计 1 分钟内 · AI 精细处理中</small>
      </div>
