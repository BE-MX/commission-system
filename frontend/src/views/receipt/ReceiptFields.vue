<template>
  <div class="receipt-fields">
    <div class="fields-grid">
      ...
      <el-form-item label="回款方式" required><el-select v-model="form.payment_type" :loading="loading" :disabled="readonly" placeholder="选择小满回款方式"><el-option v-for="t in types" :key="t" :value="t" :label="t" /></el-select></el-form-item>
      <el-form-item v-if="showCharge" label="银行手续费"><el-input-number v-model="form.bank_charge" :precision="2" :min="0" :disabled="readonly" controls-position="right" /></el-form-item>
    </div>
    <p v-if="typesError" class="types-error" role="alert">回款方式加载失败。<el-button link type="primary" @click="loadTypes">重新加载</el-button></p>
...
defineProps({ form: { type: Object, required: true }, currency: String, readonly: Boolean, showCharge: Boolean })
