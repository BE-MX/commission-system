<template>
  <div class="fx-page">
    ...
    <header class="fx-header"><div><span class="eyebrow">SETTLEMENT ADVISOR</span><h2>结汇决策助手</h2><p>用最新参考报价、你的资金期限与风险预算，找到当前条件下更合适的结汇方案。</p></div><span class="mode-label">美元 → 人民币</span></header>
    <FxMarket :market="market" :loading="marketLoading" :error="marketError" :clock="clock" @refresh="loadMarket" />
    <div class="workspace">
      <form ref="formElement" class="input-panel lg-card is-static" @submit.prevent="run(false)">
        <fieldset :disabled="!!busy">
          <legend>这笔资金的条件</legend><p class="form-intro">只填已到账资金。未来尚未收到的货款，需要单独匹配远期结汇期限。</p>
          <div class="field-pair"><label>已到账美元金额 <span>USD</span><input v-model="form.usd_balance" name="usd_balance" type="number" min="0.01" max="1000000000" step="0.01" placeholder="例如 100000" required></label><label>必须保留的美元 <span>USD</span><input v-model="form.reserved_usd" name="reserved_usd" type="number" min="0" :max="form.usd_balance || 1000000000" step="0.01" required></label></div>
          <label>现在必须补足的人民币净缺口 <span>CNY</span><input v-model="form.immediate_cny_need" name="immediate_cny_need" type="number" min="0" max="1000000000" step="0.01" required><small>已扣除现有可用人民币；这部分优先结汇。</small></label>
          <label>剩余美元最晚结汇日期<input v-model="form.settle_by" name="settle_by" type="date" :min="today" :max="maxDate" required></label>
          <label>相对全部现在结汇，可接受少得多少 <span>CNY</span><input v-model="form.max_loss_cny" name="max_loss_cny" type="number" min="0" max="1000000000" step="0.01" placeholder="例如 2000；填 0 表示不留等待敞口" required><small>用于压力情景测算，不是实际损失保证上限。</small></label>
          <label>压力情景：美元下跌幅度 <span>%</span><input v-model="form.stress_drop_pct" name="stress_drop_pct" type="number" min="0.1" max="30" step="0.1" required><small>默认 2% 是假设，可以按你的判断调整，并非预测。</small></label>
          <label class="check-label"><input v-model="manualQuote" type="checkbox" @change="setQuoteTime">填写银行实际报价</label>
          <div v-if="manualQuote" class="quote-fields"><label>实际结汇报价 <span>CNY / USD</span><input v-model="form.bank_rate" name="bank_rate" type="number" min="1" max="20" step="0.0001" placeholder="例如 6.7000" required></label><label>报价时间（北京时间）<input v-model="form.bank_quote_at" name="bank_quote_at" type="datetime-local" step="1" required><small>报价超过 15 分钟需重新核对。</small></label></div>
          <details class="advanced"><summary>利息和费用（可选）</summary><div class="field-pair"><label>美元年化利率 <span>%</span><input v-model="form.usd_interest_pct" name="usd_interest_pct" type="number" min="0" max="20" step="0.01" required></label><label>人民币年化利率 <span>%</span><input v-model="form.cny_interest_pct" name="cny_interest_pct" type="number" min="0" max="20" step="0.01" required></label></div><label>报价外额外费用 <span>基点</span><input v-model="form.fee_bps" name="fee_bps" type="number" min="0" max="1000" step="0.1" required><small>1 基点 = 0.01%；报价已含全部费用时填 0。</small></label></details>
        </fieldset>
