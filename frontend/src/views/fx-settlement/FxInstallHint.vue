<template>
  <details v-if="!standalone" class="install-hint">
    <summary><img src="/fx-app/icon-180.png" alt="" width="32" height="32" /><span>添加「结汇助手」到手机桌面</span><span class="install-action">安装指引</span></summary>
    <div class="install-body">
      ...
      <ol v-else-if="ios">
        ...
        <li>名称设为<strong>结汇决策助手</strong>。如有“作为 Web App 打开”，请开启，再点“添加”。</li>
      </ol>
      <ol v-else>
        <li>打开浏览器菜单，选择<strong>安装应用</strong>或<strong>添加到主屏幕</strong>。</li>
        <li>确认名称，完成后从桌面的结汇助手图标打开。</li>
      </ol>
      <p>使用方舟账号登录，需要结汇助手权限。行情和测算需要联网；输入与结果不会保存，关闭或刷新前请留存需要的信息。</p>
    </div>
  </details>
</template>
