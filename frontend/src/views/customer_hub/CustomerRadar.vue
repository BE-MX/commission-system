...
const drawerVisible = ref(false), workbench = ref(null), overview = ref(null)
...
function selectView(view) { if (workbench.value?.selectView) workbench.value.selectView(view) }
function onActionSaved() { workbench.value?.refresh?.(); overview.value?.refresh?.() }
</script>
<style scoped>.daily-page { display: grid; gap: 20px; color: var(--text-primary); }.daily-page h1 { margin: 0 0 8px; font-size: 17px; }.daily-page header p { margin: 0; color: var(--text-secondary); line-height: 1.6; }</style>
