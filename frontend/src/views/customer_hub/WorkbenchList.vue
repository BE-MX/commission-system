<template>
  <section class="workbench" aria-label="客户待办">
    ...
    <div class="toolbar">
      ...
      <GlassButton variant="secondary" :loading="loading" @click="fetchList">刷新</GlassButton>
    </div>
...
<style scoped>
.workbench { display: grid; gap: 14px; }.summary-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px; }.summary-card { display: grid; gap: 12px; text-align: left; padding: 18px; border: 1px solid var(--border-color); border-radius: var(--card-radius); background: var(--card-bg); color: var(--text-secondary); cursor: pointer; }.summary-card strong { color: var(--text-primary); font-size: 28px; }.summary-card.selected { border-color: var(--color-primary); background: var(--toolbar-bg); }.summary-card:focus-visible, .customer-link:focus-visible { outline: 2px solid var(--color-primary); outline-offset: 3px; }.hint { margin: 0; font-size: 12px; line-height: 1.6; color: var(--text-muted); }.toolbar { display: flex; gap: 10px; flex-wrap: wrap; }.toolbar :deep(.el-input) { flex: 1; min-width: 180px; }.toolbar :deep(.el-select) { width: 180px; }.action-list { min-height: 120px; border: 1px solid var(--border-color); border-radius: var(--card-radius); overflow: hidden; background: var(--card-bg); }.action-row { display: flex; gap: 20px; justify-content: space-between; padding: 18px; border-bottom: 1px solid var(--border-color); }.action-row:last-child { border-bottom: 0; }.row-main { min-width: 0; }.customer-link { border: 0; background: transparent; padding: 0; color: var(--color-primary); font: inherit; cursor: pointer; }.action-row h3 { margin: 8px 0; font-size: 15px; line-height: 1.5; overflow-wrap: anywhere; }.action-row p { margin: 6px 0; color: var(--text-secondary); line-height: 1.6; }.meta { display: flex; flex-wrap: wrap; gap: 14px; font-size: 12px; color: var(--text-muted); }.row-action { display: grid; align-content: center; justify-items: end; gap: 12px; flex-shrink: 0; font-size: 12px; color: var(--text-secondary); }.el-pagination { overflow-x: auto; }
