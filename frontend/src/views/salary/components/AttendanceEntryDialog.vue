<!--
  单人考勤人工录入/修正（M2-f；2026-08-07 起请假四列支持钉钉自动拉取）。

  拆成独立组件不只是为了给工作台减行数：口径说明必须跟输入框待在一起。
  请假四列现在有两个来源——同步自动拉（leave_source=dingtalk）和这里的
  人工修正；**人工改过的行同步永远让路**，所以在这里改之前要确认是比钉钉准。
...
<template>
  <el-dialog :model-value="!!row" :title="`录入考勤 — ${row?.name || ''}`" width="520px"
    ...
    <div class="hint">
      ...
      事假破全勤；病假超上限破全勤；<b>年假不破全勤</b>。改完立刻重判，不用再点别的按钮。<br>
      <b>请假四列现在会随同步自动拉取。</b>在这里改过的行就归人工管（标「手工」），
      以后同步不再覆盖它——所以只在确认比钉钉准时才改。
    </div>
