...
async function registerAndUpload(page) {
  ...
  await page.getByText('美颜生成', { exact: true }).click()
  await screenshot(page, 'capture')
  await page.setViewportSize({ width: 320, height: 568 })
  const captureLayout = await page.evaluate(() => {
    const viewport = document.querySelector('.viewport').getBoundingClientRect()
    const modes = document.querySelector('.processing-mode').getBoundingClientRect()
    return { viewportBottom: viewport.bottom, modesTop: modes.top }
  })
  assert.ok(captureLayout.modesTop >= captureLayout.viewportBottom - 1, 'photo modes reserve layout space below the camera')
  await hitTarget(page, '.processing-mode label')
  await screenshot(page, 'capture-320x568')
...
try {
  ...
  await page.locator('.matching').waitFor()
  ...
  await page.setViewportSize({width:768,height:1024})
  await page.locator('.lib-entry').click()
  await page.locator('.lib-filter-row').first().waitFor()
  assert.equal(await page.locator('.lib-filter-row').count(), 3, 'library exposes only filters backed by catalog tags')
  await page.locator('.lib-filter-row').filter({ hasText: '长度' }).getByRole('button', { name: '短发', exact: true }).click()
  assert.ok(await page.locator('.lib-card').count() > 0, 'catalog tag filter retains matching wigs')
  await page.locator('.lib-filter-row').filter({ hasText: '长度' }).getByRole('button', { name: '全部', exact: true }).click()
  await page.locator('.lib-card').last().click()
  ...
  assert.equal(generate.body.prompt_version_id, undefined)
  ...
  evidence.push('Try-on: consent, beauty mode upload, analysis, recommendation/library/swap, colors, server-default prompt generation, drag/keyboard comparison, reaction, QR, original-image print, advisor, home confirmation')
  await page.locator('.cta2').click();await registerAndUpload(page);await page.locator('.scene').waitFor()
  ...
  assert.equal(await page.getByText('出图风格', { exact: true }).count(), 0)
  ...
  evidence.push('Scenes: max 3, preserved scene_keys request, reselect, no customer style picker; reduced-motion applied')
