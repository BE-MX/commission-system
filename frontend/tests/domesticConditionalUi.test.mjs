...
test('domestic API exposes route-rule and audited skip operations', () => {
  ...
  assert.match(api, /`\/process-routes\/\$\{routeId\}\/configuration`/)
  assert.match(api, /\{ steps, rules \}/)
...
test('route management separates step and conditional-rule permissions and saves', () => {
  ...
  assert.match(view, /saveDomesticRouteConfiguration/)
  ...
  assert.match(view, /路线配置保存失败/)
  assert.doesNotMatch(view, /路线步骤已保存，但条件规则保存失败/)
