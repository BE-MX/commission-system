...
export async function installExpoFixture(page, baseURL) {
  const state = { calls: [], phase: 'pending', mode: 'tryon', polls: 0, results: [], quota: 50, failGeneration: false, displayFail: false, originalFail: false, displayMissing: false }
  ...
  await page.route(`${baseURL}/api/**`, async route => {
    ...
    if (path.endsWith('/stores/quota')) data = { bound: true, remaining: state.quota, store_name: '演示门店' }
    ...
    else if (path.endsWith('/beautify-availability')) data = { available: true, version: { id: 1, name: '强美颜·焕颜级 V1', revision: 1 } }
      ...
      state.mode = new URL(req.url()).searchParams.get('mode') || 'tryon'
      ...
      data = { status: state.phase, photo_processing_mode: 'beauty', beautify_status: 'ready', photo_url: portrait, matches: wigs, results: state.results,
