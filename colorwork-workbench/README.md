...
实时库存仍回源方舟 `/api/colorwork/inventory-status`；23 套模板按 okki 库存口径计算：库存 0 为 Restocking，1–19 为 Low Stock，20 及以上不显示提醒。
...
新版 PSD/JPG 必须相互同尺寸，允许改变 S1 画板。解析与服务端坐标校验以新版 PSD 文件头宽高为准；旧动态区域仅按宽高比例缩放参考。标题、页眉、Logo、装饰图层不作为业务色块；画布尺寸发生变化时，原当前母版中的标准色号图层按新版画布可见区域解析，未知色号或完全不可见的色号仍阻止解析。
