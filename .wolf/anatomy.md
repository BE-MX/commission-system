...
- `soul.md` — ACCIO WORK Soul (~1136 tok)
...
- `overview_service.py` — 备货管理 — 销量备货一览(SQL 分页 + 状态过滤 + 摘要统计) (~3772 tok)
- `production_cart_service.py` — 备货管理 — 生产单购物车 CRUD (~1513 tok)
- `production_order_service.py` — 备货管理 — 生产订单 CRUD + 状态管理 + 入库数量 + 审计日志 (~6645 tok)
- `router.py` — 备货管理 — API 路由 (~6258 tok)
- `safety_service.py` — 备货管理 — 安全库存 CRUD + AI 备货建议(TFT 微服务 / 公式兜底) (~4175 tok)
...
- `index.js` — NAV_ENTRIES 中每条记录映射成 vue-router 的 children 路由 (~676 tok)
...
- `LoginPage.vue` — Vue: setup (~2442 tok)
