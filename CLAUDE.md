...
- **素材管理（7 张表，020 迁移）**：
  - `ark_tag_dimensions` — 标签维度（name, label, is_single_select, is_system, is_required, sort_order）
  - `ark_tag_values` — 标签值（dimension_id, value, color_hex, sort_order, is_active）
  - `ark_assets` — 素材主表（file_name, file_type, file_format, storage_path, file_size, thumbnail_path, uploader_id, current_version_id, status: latest/history/offline, download_count）
  - `ark_asset_versions` — 版本历史（asset_id, version_number, storage_path, file_size, uploader_id, remark）
  - `ark_asset_tags` — 素材-标签关联（asset_id, version_id, dimension_id, tag_value_id）
  - `ark_asset_permissions` — 权限（asset_id, permission_group: all/design_dept/sales/specific, allow_preview, allow_download, specified_user_ids JSON）
  - `ark_favorite_folders` — 收藏夹（user_id, name, sort_order, share_token, share_expires_at）
  - `ark_favorite_items` — 收藏项（folder_id, asset_id, sort_order）
  - `ark_download_logs` — 下载日志（asset_id, user_id, version_number）
...
**定时任务**：`backend/app/schedulers/registry.py` 在 `start_scheduler()` 中创建 `AsyncIOScheduler` 并注册七个 job（main.py 仅调用 `start_scheduler()`/`shutdown_scheduler()`,任务定义集中在 registry）：
