...
- **bat 脚本**用 UTF-8 编码 + 文件首行 `chcp 65001 >nul`（与现有 setup-server.bat / restart.bat 一致），变量赋值用 `set VAR=value`（不带引号），否则 Windows cmd 解析出错
...
- **前端大页面拆分模式**:vue 文件大(> 500 行)时优先抽 `composables/use<Page>.js`(全部 state + 业务方法),主文件 script setup 留薄壳(import + destructure)。template / style 内联保留,单页面密集样式不强求复用。独立 UI 大块(如 600 行 SVG 动画弹窗)拆 `components/<Block>.vue`。完成参考:tracking/composables/{useWaybillUpload,useShippingDailyReport} + tracking/components/WaybillSuccessDialog,design/composables/useDesignManage,insight/composables/useCaseLibrary,dashboard/composables/useDashboardData,system/composables/useAiManager,composables/useTableSort（共享排序 composable,分页表 `sortable="custom"` + `@sort-change` 服务端排序,静态表直接 `sortable` 客户端排序）。
...
- **测试 infra**:`backend/pytest.ini` 配 `asyncio_mode=auto`(async 测试无需 `@pytest.mark.asyncio` decorator)。`tests/conftest.py` 已加 SQLite-MySQL 兼容 hook(`BigInteger/LONGTEXT/MEDIUMTEXT` 编译 + 同名 Index dedup)。**写 scheduler 测试时**:`SessionLocal` 必须 monkeypatch 在 job 模块的导入点(如 `app.design.scheduler.SessionLocal`),不能 patch `app.core.database.SessionLocal`(后者只影响新 import)。当前覆盖:提成 27 + design 状态机/冲突引擎 34 + scheduler smoke 10 = **71 tests**;tracking 轮询 / insight 完整链路 / stock 计算 / design router 端到端 仍欠。
