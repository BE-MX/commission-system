...
def test_build_push_payload_stock_custom_and_backfilled(db):
    ...
    # 未回填 custom 行：通用产品 + 真实描述透传
    assert rows[1]["product_id"] == 888 and rows[1]["sku_id"] == 999
    assert rows[1]["product_name"] == "Genius Weft/18/#1/20g"
    assert rows[1]["product_model"] == "B1"
    assert rows[1]["unit"] == "20g"
    # 已回填 custom 行：转正走真实 ID，不再透传描述
    assert rows[2]["product_id"] == 555 and rows[2]["sku_id"] == 556
    assert "product_name" not in rows[2]
...
def test_sync_invoice_success_state_and_unique_id_writeback(db, monkeypatch, no_reconcile):
    ...
    invoice = _make_invoice(db)
    invoice.items.append(_stock_item())
    # 两条 custom 行共用通用产品 888/999 —— 响应按组内顺序认领 unique_id
    ...
    def fake_push(db_, payload):
        ...
        return {
            ...
            "product_list": [
                ...
                {"unique_id": 223, "product_id": "888", "sku_id": "999"},
            ],
        }
...
def test_edit_push_blocks_multiple_fresh_generic_rows(db):
    # 编辑推单一次新增 ≥2 条共用通用产品的行 → OKKI 按 product+sku 去重塌行，前置拦截
    _seed_settings(db)
    _seed_binding(db)
    ...
    # 首推（无 order_id）不拦截：OKKI 新建时不去重，且响应会发 unique_id
