...
def install_memory_provider(db, configuration, settings, monkeypatch):
    ...
    for name, cap in ((settings.WHATSAPP_REPLY_PLANNER_PRESET, 1400), (settings.WHATSAPP_REPLY_GENERATOR_PRESET, 1800)):
        ...
        preset.parameters = {**configuration["parameters"], "max_tokens": cap}
    db.commit()
...
def evaluate_cases(db, identity, make_request):
    if db.get_bind().url.drivername != "sqlite" or db.get_bind().url.database != ":memory:":
    ...
    summary = {
        ...
        "median_ms": (durations[14] + durations[15]) / 2, "p95_ms": durations[28],
