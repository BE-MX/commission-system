...
class Invoice(Base):
    ...
    okki_new_deal = Column(SmallInteger, nullable=True, comment="OKKI必填-是否新成交:1是/0否/NULL推单时按客户有无okki历史订单兜底")
    ...
    source_type = Column(String(32), nullable=False, default="manual", server_default="manual", comment="来源：manual/okki_screenshot/external_api")
    source_order_id = Column(String(64), nullable=True, comment="OKKI订单ID、截图幂等键或外部接入请求ID")
    ...
    sync_attempt = Column(JSON, nullable=True, comment="普通推送持久发送令牌与租约")
    cancellation = Column(JSON, nullable=True, comment="取消流程持久状态、远端核对结果与审计依据")
    outbound_auto_requested = Column(SmallInteger, nullable=False, default=0, server_default="0", comment="是否已登记自动出库意图；不自动回填历史订单")
...
class OkkiOutboundTask(Base):
    ...
    状态机：pending/waiting_stock → running → done/skipped/waiting_stock/failed/uncertain；
    waiting_stock 每15分钟按目标仓库可用库存复查，不限次数；明确库存拒绝才允许进入。
    failed 且 attempts 未超限由轮询器按
