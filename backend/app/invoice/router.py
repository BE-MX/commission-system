...
class LinkedSavePayload(BaseModel):
...
def _linked_result(row, invoice, user):
    ...
    result = linked.describe(row)
    action = "super_admin" in user.get("roles", []) or bool(
        {"receipt:read", "receipt:write", "receipt:sync", "receipt:admin"} & set(user.get("permissions", [])))
    if result and not (action and (access.all_access(user) or invoice.sales_user_id == access.user_id(user))):
        result["steps"] = {**result["steps"], "receipt": {"status": result["steps"]["receipt"]["status"],
            "message": "回款已按实际记录核对，详细金额需回款数据权限"}}
    return result


@router.post("/invoices/{invoice_id}/linked-sync", summary="Save invoice and register linked synchronization")
def save_linked(invoice_id: int, body: LinkedSavePayload, db: Session = Depends(get_db),
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(409, str(exc) if isinstance(exc, ValueError) else "提交标识已被使用，请刷新") from exc
    return ok({"operation": _linked_result(row, invoice, user), "invoice": service.serialize_detail(invoice, db)})
...
def close_linked(invoice_id: int, identity: str, db: Session = Depends(get_db),
    ...
    if row is None or row.invoice_id != invoice.id:
        raise HTTPException(404, "关联同步记录不存在")
    try:
        row = linked.close_failed(db, identity)
    except ValueError as exc:
        db.rollback()
        raise HTTPException(409, str(exc)) from exc
    return ok(_linked_result(row, invoice, user))
...
class LinkedResolutionPayload(BaseModel):
    reason: str = Field(min_length=10, max_length=500)
    confirmed: bool


@router.post("/invoices/{invoice_id}/linked-sync/{identity}/resolve", summary="Record administrator manual reconciliation")
def resolve_linked(invoice_id: int, identity: str, body: LinkedResolutionPayload, db: Session = Depends(get_db),
    ...
    if not body.confirmed:
        raise HTTPException(400, "请先人工核对小满订单、出库及回款原单")
    try:
        row = linked.resolve_manually(db, identity, _user_id(user), body.reason.strip())
    except ValueError as exc:
        db.rollback()
        raise HTTPException(409, str(exc)) from exc
    return ok(_linked_result(row, invoice, user))
