...
class ProcessRouteStep(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    route_id = Column(Integer, ForeignKey("process_route.id", ondelete="CASCADE"), nullable=False, comment="关联路线ID")
    process_id = Column(Integer, ForeignKey("process.id", ondelete="RESTRICT"), nullable=False, comment="关联工序ID")
...
class ProductProcessRoute(Base):
    ...
    route_id = Column(Integer, ForeignKey("process_route.id", ondelete="RESTRICT"), nullable=False, comment="关联路线ID")
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at = Column(DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
