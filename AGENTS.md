...
13.5 **原型设计 / UI 动效任务强制加载 Emil Kowalski skill 组**：动手前先 `Skill: emil-design-eng`（UI 打磨与动效决策哲学）；动效代码完工后按 `~/.Codex/skills/review-animations/STANDARDS.md` 逐条自查（该 skill 限用户 `/review-animations` 手动触发，模型侧读标准文件执行同级审查）；描述不清的动效效果用 `animation-vocabulary` 定位术语
...
22. **多 AI agent 共享同一 git 工作树（Codex 与 Claude 并行，共用一个 `.git/HEAD`）**：切分支会互相影响，务必遵守——① **各自只在自己分支干活**：动手改代码前先 `git checkout -b codex/<topic>`，**绝不直接在 main 上提交**（背景：2026-07-14 本进程在 main 上连提 4 个 docs 后才切分支，把并行 Claude 的提交挤到了错误分支、造成 main 与云端不一致）；② 不要切换共享工作树的当前分支去操作别的 agent 的分支；③ 每次 `commit`/`push` 前先 `git branch --show-current` 确认在自己分支，`push` 后用 `git cat-file -s origin/<branch>:<path>` 核对推上去的确是自己的改动；④ 落错分支只用 fast-forward 或 `git update-ref` 无损嫁接，**绝不 `reset`/`rebase` 别人的提交**；⑤ 要强隔离就各自开独立 `git worktree`（`git worktree add ../commission-system-codex codex/<topic>` 在自己目录里跑）。对应教训见 `.wolf/cerebrum.md` 2026-07-14 条目。
